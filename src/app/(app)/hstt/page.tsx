      ...
      <section className="card overflow-hidden">
        ...
        {daGui?.length ? (
          <div className="overflow-x-auto">
            <table className="tbl">
              <thead>
                <tr><th>Khách hàng</th><th>Kỳ</th><th>Tệp đã gửi</th><th className="text-right">Gửi ngày</th></tr>
              </thead>
              <tbody>
                {daGui.map((r: any) => (
                  <tr key={r.id}>
                    ...
                    <td className="mono text-[12px]">{r.ky_doi_soat}</td>
                    <td className="text-[12.5px]">{r.ten_file_hstt_da_gui ?? '—'}</td>
                    <td className="text-right mono text-[12px] text-[var(--ink-3)]">
                      {r.ngay_gui_gan_nhat ?? '—'}
                    </td>
                  </tr>
